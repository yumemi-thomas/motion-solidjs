...
// so these shapes fade in via opacity instead of being stroked on.
...
export default function SvgPathShapes() {
  return (
    <div class="flex flex-col items-center gap-3">
      <svg viewBox="0 0 600 600" class="h-64 w-64">
        <For each={shapes}>
          {(s) => {
      ...
      <TodoNotice>
        solidjs-motion defers SVG path drawing (<code>pathLength</code>) to v0.3+ — shapes fade in
        via opacity instead of being stroked on.
      </TodoNotice>
    </div>
  )
}
