...
plus animation props such as `initial`, `animate`, `transition`, `hover`, `press`,
`inView`, `drag`, `layout`, and `variants`.
...
- `exit` - style applied when leaving an `<Presence>` tree
- `transition` - `tween`, `spring`, or `inertia` options
- `hover` / `press` / `focus` / `inView` - gesture variants
...
- [`Presence`](/docs/animate-presence) - animate elements as they leave the tree
