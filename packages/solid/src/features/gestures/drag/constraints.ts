...
// exported by motion-dom) — check drift when bumping the motion peer range.
...
/**
 * Calculate the projection point for a min/start point when dragging from
 * `progress` along an axis whose total length is `length`, with optional
 * constraints + elastic.
 *
 * E.g. element is 100px wide, dragging from 0.25 along this axis, pointer at
 * 200px, no constraints → min projection 175.
 */
...
/**
 * Calculate an axis position based on two axes and a progress value.
 */
