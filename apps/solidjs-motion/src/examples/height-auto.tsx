...
export default function HeightAuto() {
  ...
  return (
    <div class="w-64">
      <button
        onClick={() => setOpen((v) => !v)}
        class="w-full rounded-2xl border border-border bg-card px-4 py-2.5 text-sm font-medium"
      >
      ...
      <Presence>
        <Show when={open()}>
          <motion.div
            class="mt-2 overflow-hidden rounded-2xl border border-border bg-card text-xs text-fg-muted"
            initial={{ height: 0, opacity: 0 }}
            animate={{ height: 'auto', opacity: 1 }}
            exit={{ height: 0, opacity: 0 }}
            transition={{ type: 'spring', stiffness: 280, damping: 28 }}
          >
            <div class="space-y-2 px-4 py-3 leading-relaxed">
